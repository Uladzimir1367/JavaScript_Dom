...
// DOMContentLoaded используем для запуска функции, которая добавляет обработчики событий на элементы,
// только после полной загрузки DOM. Обернём наш скрип в эту функцию!
// Добавим в HTML для span - id = t. Находим элементы по их id.
// (Для станицы это будут уникальные элементы). Код сложнее, чем был на семинаре, но строчек меньше !!!
...
// Для выполнения данной задачи в HTML проще всего подключить напрямую библиотеку стилей.
// Лучше прописать стили в отдельном файле СSS.
// Класс animate__animated является общим классом для анимации из библиотеки Animate.css, 
//а animate__fadeInLeftBig - это конкретный класс, который определяет тип анимации 
// (в данном случае, появление элемента с левой стороны с большим смещением).
...
document.addEventListener("DOMContentLoaded", () => {
   ...
   //  обращение напрямую на странице
 ...
 // эта часть скрипта обеспечивает обратную связь и проверяет, что поля формы заполнены корректно, 
 // подсвечивая незаполненные поля и убирая подсветку, как только поле начинает заполняться.
 //  Это помогает предотвратить попытку отправки формы с незаполненными полями 

  formEl.addEventListener("input", (e) => {
    const target = e.target;
     //  обработчик события input ко всей форме. Событие input срабатывает каждый раз,
     // когда пользователь вводит что-то в любое поле ввода внутри формы. 
